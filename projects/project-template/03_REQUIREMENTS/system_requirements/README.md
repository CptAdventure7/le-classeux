...
- Keep one top-level `system_requirement.json` file in this folder when real content exists.
...
- `system_requirement.json` must be a top-level JSON array where each object is one requirement.

## Authoring Rules

- Use IDs with the `PRJ-REQ-` prefix.
- Requirement `status` must be one of `Accepted`, `Preliminary`, or `Abandoned`.
- Obsolete or non-relevant requirements must remain in `system_requirement.json` and be retagged as `Abandoned`; do not delete them from the live record.
...
```json
{
  ...
  "reference": [
    "2026-03-23 | source | 03_REQUIREMENTS/user_requirements/user_requirements.json#PRJ-UN-3"
  ],
