...
Artifacts specifically related to 03 Requirements.
...
- Review adjacent folders when a change affects related material: `01_PROJECT_FOUNDATION`, `02_SYSTEM_DEFINITION`, `04_DESIGN_AND_IMPLEMENTATION`, `05_EXPERIMENTS_AND_VALIDATION`, `08_DECISIONS`.
...
- Store canonical requirements in the single `requirements.json` file of the relevant child folder: `user_requirements`, `system_requirements`, or `subsystem_requirements`.
- Keep each `requirements.json` file as a top-level JSON array where each item is one requirement object.
- Use scope-specific ID prefixes:
  `PRJ-UN-` in `03_REQUIREMENTS/user_requirements/requirements.json`
  `PRJ-REQ-` in `03_REQUIREMENTS/system_requirements/requirements.json`
  `PRJ-SUBREQ-` in `03_REQUIREMENTS/subsystem_requirements/requirements.json`
